...
### Model 10 — multi-agent LLM trading council (live forward paper-trading study)

Prompted by the question of whether an *agentic* system — LLM agents that reason, debate, and pull in information — could find what six trained models could not. Researched before building, as instructed.

**Literature reviewed:**
- **TradingAgents** (Xiao et al., [arXiv:2412.20138](https://arxiv.org/abs/2412.20138), UCLA/MIT/Tauric Research, [code](https://github.com/TauricResearch/TradingAgents)) — specialist analysts feed a bull-vs-bear debate, then a trader and risk manager. This decomposition is the genuinely valuable idea and is the architectural basis for what was built here.
- **FinMem** ([arXiv:2311.13743](https://arxiv.org/abs/2311.13743), [code](https://github.com/pipiku915/FinMem-LLM-StockTrading)) — evaluated and **rejected for two concrete reasons**, not on taste: it hard-requires `OPENAI_API_KEY` for `text-embedding-ada-002` regardless of backbone LLM (no free path), and its reference config bakes hindsight into the agent's persona (`"Tesla's continued growth ... despite certain quarters where revenue missed Wall Street expectations"` — describing the very test period), violating the point-in-time discipline enforced everywhere else in this project. Its headline result (61.78% return, Sharpe 2.68) comes from a **3.5-month single-ticker window** — a smaller sample than the one Phase 26 already proved unreliable here.
- **Field audit** ([arXiv:2605.19337](https://arxiv.org/abs/2605.19337)) — screened 77 LLM-trading studies. Of the 19 meeting minimum standards: only **2** had extractable time-consistent train/test splits, only **1** modelled transaction costs, only **1** documented survivorship-bias handling, and **none** reached full reproducibility. Its verdict on the field's reported results: not credible as currently reported.

**The decisive data constraint, verified rather than assumed.** The keyless news source returns only articles published in roughly the **last two days** (checked directly: every article for CL=F and GC=F was dated within 48 hours). TradingAgents' own news fetcher is correctly look-ahead-safe — it filters articles to the requested date window — which means on any historical backtest date it returns "No news found" and the news, sentiment, and debate agents reason over *nothing* while appearing to run fine. **A historical backtest of any news-driven agent on free data is therefore not merely noisy, it is vacuous.** Feeding today's news against a past date instead would be catastrophic look-ahead bias. Forward paper-trading is the only honest option, so that is what was built.

**ZN=F excluded**: Yahoo returns **zero** news articles for Treasury futures (verified). Running the council on it would fabricate a third data point from an empty news set.

**Why the off-the-shelf framework was rebuilt rather than used.** Running TradingAgents as-shipped produced a diagnosed, instructive failure on its very first live run. Its technical analyst was handed a raw price CSV and responded with **a pandas tutorial** (*"to correctly parse your data ... use `pd.read_csv`"*) instead of an analysis. With no price anchor reaching downstream, the bull/bear/risk debate **invented one** — arguing *"selling CL=F at 95 with a stop at 85"* while crude traded at **82.28**. The output was confident, well-structured, cited real news, and was numerically fictional. Root cause was not model size: it was handing a language model a table and expecting arithmetic. Its tooling is also equity-shaped (`get_balance_sheet`, StockTwits, insider transactions are meaningless or 404 for `CL=F`), so roughly half its analyst roster contributes nothing on futures.

**What was built** (`backend/app/agent/`):
- **`brief.py`** — does all numerical work in verified Python and hands the model finished figures in prose: 25-feature technical engine (incl. Baz et al. trend scores), real EIA/FRED macro, real CFTC positioning with the new 3-year percentile extremity transform, and real current headlines. Crucially this is the **same information set Models 4-9 used**, making "council vs. trained models" an honest comparison rather than a confounded one.
- **`council.py`** — 7 agents: technical / macro / positioning / news analysts → bull researcher → bear researcher rebutting → risk manager returning structured `ACTION / CONFIDENCE / KEY_LEVEL / REASONING`. Runs entirely on local Ollama: free, no API key, no data leaves the machine.
- **`scripts/paper_trade_agent.py`** — daily forward runner; logs each decision with the decision-time price and a full snapshot of the facts the agent saw, so any decision can be audited later against exactly what it knew.
- **`scripts/evaluate_paper_trades.py`** — joins decisions to realized forward returns vs. buy-and-hold, and **refuses to report performance statistics below 20 resolved observations per market**, enforced in code rather than left to judgment in the moment.

**First verified council decision**: CL=F **HOLD / LOW confidence** at 82.45, with every cited figure traceable to the brief (RSI 60.9, MACD +0.799, 1-ATR range 79.41-85.49, USD index 118.06, speculators at the 25th percentile). Zero hallucination, against the same framework's `95/85` fiction the day before. Its reasoning correctly noted positioning was *mid-range and therefore uninformative* and declined to force a trade. **One grounded decision is not evidence of predictive skill** — that requires the accumulating forward log, and the evaluator will not pretend otherwise.

**The price-anchor guard, and two bugs in the guard itself.** Because instructions reduce hallucination but never prove its absence, every decision is verified programmatically against the real price. That guard was wrong twice, and both times was caught by testing it against real cases rather than trusting it:
1. **v1 checked whether *any* cited price was close** — and passed the very hallucination it was written to catch, since $85 is only 3.3% from $82.28 while the fictional $95 entry went unnoticed. Fixed to check the *farthest* cited price.
2. **v2 swept up every number in a plausible band** — and flagged a perfectly grounded decision because it quoted RSI 60.7, 42% volatility, and the 118.06 dollar index, none of which are prices. **A guard that discards valid decisions corrupts the study as surely as one that admits invalid ones.** Fixed to require price-like context and to exclude any figure matching a known non-price metric from the brief.
